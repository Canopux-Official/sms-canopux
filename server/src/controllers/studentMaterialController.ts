...
const showClass = async (req: AuthRequest, res: Response) => {
    try {
        const organizationId = req.user?.organizationId;
        const studentId = req.user?.id;

        const student = await Student.findOne({ _id: studentId, organizationId })
            .populate('targetExams', 'name _id')
...
const getRecentMaterials = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const student = await Student.findOne({ _id: userId, organizationId })
...
const getMaterialStats = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        const organizationId = req.user?.organizationId;
