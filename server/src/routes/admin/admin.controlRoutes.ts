...
router.get('/getAllAdmins', verifyAuth, requireSuperAdmin, async (req: AuthRequest, res: express.Response): Promise<void> => {
    try {
        const admins = await Admin.find({ role: { $in: ['admin', 'superadmin'] }, organizationId: req.user?.organizationId }).select('-password');
...
router.put('/updatePermissions/:id', verifyAuth, requireSuperAdmin, async (req: AuthRequest, res: express.Response): Promise<void> => {
    try {
        ...
        const updatedAdmin = await Admin.findOneAndUpdate(
            { _id: id, role: { $in: ['admin', 'superadmin'] }, organizationId: req.user?.organizationId },
            { $set: { permissions } },
            { new: true }
        ).select('-password');
...
router.put('/updateAdmin/:id', verifyAuth, requireSuperAdmin, async (req: AuthRequest, res: express.Response): Promise<void> => {
    try {
        ...
        const admin = await Admin.findOne({ _id: id, organizationId: req.user?.organizationId });
        if (!admin) {
        ...
        if (admin.role === 'superadmin') {
            admin.permissions = {
                ...
                notice: true, attendance: true, marks: true
            };
        }
...
router.delete('/deleteAdmin/:id', verifyAuth, requireSuperAdmin, async (req: AuthRequest, res: express.Response): Promise<void> => {
    try {
        ...
        const deletedAdmin = await Admin.findOneAndDelete({ _id: id, organizationId: req.user?.organizationId });
